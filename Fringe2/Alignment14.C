...
void Alignment( string filefolder = "170211/", string filebase = "print_011_", int fCut = 100, int fCut_scan = 100){
  ...
  TH1D *hnoise_tgt  = new TH1D("hnoise_tgt","noise target",210,-1.,11.);
  ...
  cout << "ipeak, time[sec], voltage[V], ono[um],LP voltage[V], ono_scan[um]  " << endl;
  ...
  TH1D *hs_full = new TH1D("hs_full","Full bin _ Average of Scanning stage",n,RangeMin,RangeMax);
  // double sum = 0;
  // double ave = 0;
  // std::vector<double> sum[ n/div ] = 0;
  // std::vector<double> ave[ n/div ] = 0;
  // std::vector<double> ave_diff[ div ] = 0;
  double sum[ 999999 ] = {};
  double ave[ 999999 ] = {};
  double ave_diff[ 999999 ] = {};
  for(int i=0; i < n/div ; i++){
      for(int isum=0; isum<div ; isum++){
          ...
          sum[ i ] += hraw_scan->GetBinContent(i*div+isum);
          cout << isum << " " << hraw_scan->GetBinContent(i*div+isum) << " " << sum[ isum ] << endl;
      }
      ave[ i ] = sum[ i ]/div;
      hs->SetBinContent(i,ave);
      cout << i*div << " " << sum[ i ] << " " << ave[ i ] << endl;
      if(i!=0){
          for( int ifull = 0; ifull < div ; ifull++){
              ave_diff[ ifull ] = (ave[ i ] - ave[ i-1 ])/div;
              hs_full->SetBinContent((i - 1)*div + ifull, ave[i]-ave_diff*ifull)
                  }
      }
      // sum = 0;
      // ave = 0;
