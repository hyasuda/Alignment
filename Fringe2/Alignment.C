...
void Alignment(int fCut = 100){
    ...
    //    ifstream data("tek0001.dat");
    ifstream data("/Users/YASUDA/Data/Alignment/170131/tek0001.dat");
    ...
    for(int i=0;i<n;i++){
        ...
        if(diff_prevsw){
            if(diff_prev < 0.0) diff_prevsw = 0;
        } else {
            if(diff_prev > 20.0) diff_prevsw = HistoMax;
        }

        h_diffsw->SetBinContent(i+1,diffsw);
        h_diffsw2->SetBinContent(i+1,diffsw);
        //        cout << i << " " << diff_prev << " " << diff_prevsw << " " << diff << " " << diffsw << endl;
        cout << i << " " << diffsw << " " << diff_prevsw << endl;
	if ( diff_prevsw == HistoMax && diffsw == 0 ) {
	  bin_peak = i;
	}
    ...
    if ( bin_peak < 0 ) {
      ...
      t_peak = (double) vx[ bin_peak ];
      v_peak = (double) vy2[ bin_peak ];
    }
    cout << "bin_peak is " << bin_peak << endl;
    cout << "t_peak is " << t_peak <<endl;
    cout << "v_peak is " << v_peak <<endl;
    ...
    hsnoise->SetTitle("Scanning Stage Noise");
